...
void IRTranslator::pConstDefTranslator(IRStatement& statement) {//对于函数参数对于栈空间的占用，调用函数进入新的复合语句块之后先把函数参数取出之后再声明其他常量或者变量
    ...
    outMips("li $t8," + value);
    outMips("sw $t8,-" + to_string(this->curStackAlloc) + "($sp)");
...
void IRTranslator::nonRetFuncCallTranslator(IRStatement& statement,unsigned int beginloc){
    string functionName;
    stringstream ss(statement.getIR());
...
void IRTranslator::retFuncCallTranslator(IRStatement& statement, unsigned int beginloc){
    unsigned int reg;
    string retVar;
    string functionName;
    stringstream ss(statement.getIR());
...
void IRTranslator::geqJumpTranslator(IRStatement& statement) {
    ...
    getline(ss,right,' ');
    getline(ss,label,' ');
    if (isConstantString(left) || isConstantString(right)) {
        if (isConstantString(left) && isConstantString(right)) { // 两个比较数都是非零常数
            outMips("li $t8," + to_string(stringToNumber(left)));
            outMips("bge $t8," + to_string(stringToNumber(right)) + "," + label);
        } else {
            if (isConstantString(left)) {
                regNum reg = tmpRegisterPool(right, true);
                outMips("ble $" + to_string(reg) + "," + to_string(stringToNumber(left)) + "," + label);
            } else {
                regNum reg = tmpRegisterPool(left, true);
                outMips("bge $" + to_string(reg) + "," + to_string(stringToNumber(right)) + "," + label);
            }
        }
        return;
    }
...
void IRTranslator::greJumpTranslator(IRStatement& statement) {
    ...
    if (isConstantString(left) || isConstantString(right)) {
        if (isConstantString(left) && isConstantString(right)) {
            outMips("li $t8," + to_string(stringToNumber(left)));
            outMips("bgt $t8," + to_string(stringToNumber(right)) + "," + label);
        } else {
            if (isConstantString(left)) {
                regNum reg = tmpRegisterPool(right, true);
                outMips("blt $" + to_string(reg) + "," + to_string(stringToNumber(left)) + "," + label);
            } else {
                regNum reg = tmpRegisterPool(left, true);
                outMips("bgt $" + to_string(reg) + "," + to_string(stringToNumber(right)) + "," + label);
            }
        }
        return;
    }
...
void IRTranslator::lesJumpTranslator(IRStatement& statement) {
    ...
    if (isConstantString(left) || isConstantString(right)) {
        if (isConstantString(left) && isConstantString(right)) { // 两个比较数都是非零常数
            outMips("li $t8," + to_string(stringToNumber(left)));
            outMips("blt $t8," + to_string(stringToNumber(right)) + "," + label);
        } else {
            if (isConstantString(left)) {
                regNum reg = tmpRegisterPool(right, true);
                outMips("bgt $" + to_string(reg) + "," + to_string(stringToNumber(left)) + "," + label);
            } else {
                regNum reg = tmpRegisterPool(left, true);
                outMips("blt $" + to_string(reg) + "," + to_string(stringToNumber(right)) + "," + label);
            }
        }
        return;
    }
...
void IRTranslator::leqJumpTranslator(IRStatement& statement) {
    ...
    if (isConstantString(left) || isConstantString(right)) {
        if (isConstantString(left) && isConstantString(right)) { // 两个比较数都是非零常数
            outMips("li $t8," + to_string(stringToNumber(left)));
            outMips("ble $t8," + to_string(stringToNumber(right)) + "," + label);
        } else {
            if (isConstantString(left)) {
                regNum reg = tmpRegisterPool(right, true);
                outMips("bge $" + to_string(reg) + "," + to_string(stringToNumber(left)) + "," + label);
            } else {
                regNum reg = tmpRegisterPool(left, true);
                outMips("ble $" + to_string(reg) + "," + to_string(stringToNumber(right)) + "," + label);
            }
        }
        return;
    }
...
void IRTranslator::eqlJumpTranslator(IRStatement& statement) {
    ...
    if (isConstantString(left) || isConstantString(right)) {
        if (isConstantString(left) && isConstantString(right)) { // 两个比较数都是非零常数
            outMips("li $t8," + to_string(stringToNumber(left)));
            outMips("beq $t8," + to_string(stringToNumber(right)) + "," + label);
        } else {
            if (isConstantString(left)) {
                regNum reg = tmpRegisterPool(right, true);
                outMips("beq $" + to_string(reg) + "," + to_string(stringToNumber(left)) + "," + label);
            } else {
                regNum reg = tmpRegisterPool(left, true);
                outMips("beq $" + to_string(reg) + "," + to_string(stringToNumber(right)) + "," + label);
            }
        }
        return;
    }
...
void IRTranslator::neqJumpTranslator(IRStatement& statement) {
    ...
    if (isConstantString(left) || isConstantString(right)) {
        if (isConstantString(left) && isConstantString(right)) { // 两个比较数都是非零常数
            outMips("li $t8," + to_string(stringToNumber(left)));
            outMips("bne $t8," + to_string(stringToNumber(right)) + "," + label);
        } else {
            if (isConstantString(left)) {
                regNum reg = tmpRegisterPool(right, true);
                outMips("bne $" + to_string(reg) + "," + to_string(stringToNumber(left)) + "," + label);
            } else {
                regNum reg = tmpRegisterPool(left, true);
                outMips("bne $" + to_string(reg) + "," + to_string(stringToNumber(right)) + "," + label);
            }
        }
        return;
    }
...
void IRTranslator::flushRegMap() {
    ...
    if (this->curFunctionName != "__end")
        for (auto it = this->varRegisterMap.begin();it != this->varMemoryMap.end();it++) {
            name = it->first;
            if (this->table->isPartialVariable(name, this->curFunctionName)) {
                this->table->setPartialVariableRefreshed(name, this->curFunctionName);
            }
            if (this->table->globalExist(name)) {
                this->table->setGlobalVariableRefreshed(name);
            }
        }
    ...
    for (unsigned int i = 0;i < blk.size();i++) {
        ...
        switch (statement.getIRType()) {
            ...
            case funcDefLabel:
                ...
                break;
            case nonRetFuncCall:
                nonRetFuncCallTranslator(statement, i);
                break;
            case retFuncCall:
                retFuncCallTranslator(statement, i);
                break;
            case funcDefPara:
