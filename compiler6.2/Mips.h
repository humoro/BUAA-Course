...
class BasicClassifier{
    ...
    ofstream& irFile;
public:
    explicit BasicClassifier(vector<BasicBlk>& blks,  ofstream& irFile) : blks(blks) , irFile(irFile){}

    void outputIR(IRSyntaxNode* irSyntaxNode);
...
class IROptimal{//中间代码块优化
    SymbolTable* table;
public:
    explicit IROptimal(SymbolTable* table) : table(table){}

    void optimizeIR(vector<BasicBlk>& blks);

    void copyDiffuse(IRSyntaxNode *node);

    void constantDiffuse(vector<BasicBlk>& blks);
};
...
public:
    explicit MipsGenerator(ofstream& mipsFile, ofstream& irFile, SymbolTable* table, IRSyntaxNode* tree) {
        this->classifier = new BasicClassifier(blks , irFile);
        this->optimal = new IROptimal(table);
