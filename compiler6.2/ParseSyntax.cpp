...
void SyntaxParser::constDeclarationParser(IRSyntaxNode *parentNode) {//解析常量声明
    auto* kid = new IRSyntaxNode(Non_Const_Decl);
...
void SyntaxParser::constDefinitionParser(IRSyntaxNode *parentNode) {//解析常量定义
    auto* kid = new IRSyntaxNode(Non_Const_Def);
    parentNode->addKid(kid);
    string ans;
    string varName;
    int varValue  = -1;
    ...
    while (type != SEMICN) {
        ...
        constDefIR(varName, varValue, kid);//生成常量声明的中间代码
        flushStreamPos(ps);
        type = getNextSym(ans);
        if (type != SEMICN && getLineNumber() > ps->getLineNumber()) {//没有分号已经解析到下一行
            callBackStreamPos(ps);
            break;
        }
    }
...
void SyntaxParser::variableDeclarationParser(IRSyntaxNode *parentNode) {//解析变量声明
    auto* kid = new IRSyntaxNode(Non_Var_Decl);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::variableDefinitionParser(IRSyntaxNode *parentNode) {//解析变量定义
    auto* kid = new IRSyntaxNode(Non_Var_Def);
    parentNode->addKid(kid);
    string ans;
    TokenType type;
    auto* ps = getCurPosition();
    ...
    while (type != SEMICN) {
        ...
        if (type != LBRACK) {
            ...
            variableDefIR(varName, size, kid);//一维变量的声明
            continue;
        }
        ...
        variableDefIR(varName, size, kid);//数组的声明
        flushStreamPos(ps);
        type = getNextSym(ans);
        if (type != SEMICN && getLineNumber() > ps->getLineNumber()) {//没有分号，解析到下一行，需要回退
            callBackStreamPos(ps);
            break;
        }
    }
...
void SyntaxParser::returnFuncDefinitionParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Return_Func);
    parentNode->addKid(kid);
    string ans;
    TokenType type = getNextSym(ans);
    string functionName;
    ...
    addFunctionNameLabelParser(functionName, kid);
...
void SyntaxParser::voidFuncDefinitionParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Void_Func);
    parentNode->addKid(kid);
    string ans;
    TokenType type = getNextSym(ans);
    string functionName;
...
void SyntaxParser::addFunctionNameLabelParser(string &functionName, IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Func_Label);
    parentNode->addKid(kid);
    functionDefIR(functionName, kid);//中间代码函数标签
}
...
void SyntaxParser::funcParameterListParser(IRSyntaxNode *parentNode) {//解析函数参数表
    auto* kid = new IRSyntaxNode(Non_Para_Table);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    ...
    if (varType == RPARENT || varType == LBRACE) {//没有右括号的时候终止需要通过达到大括号来判断
        ...
        for (;;) {
            ...
            flushStreamPos(ps);
            functionDefParaIR(ans, kid);//参数的中间代码
...
void SyntaxParser::compoundStatementParser(string &functionName, IRSyntaxNode *parentNode) {//复合语句的解析程序
    auto* kid = new IRSyntaxNode(Non_Cmpd_Stat);
    parentNode->addKid(kid);
...
void SyntaxParser::statementColumnParser(IRSyntaxNode *parentNode) { // 语句列
    auto* kid = new IRSyntaxNode(Non_Stat_Column);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::statementParser(IRSyntaxNode *parentNode) {//语句解析
    auto* kid = new IRSyntaxNode(Non_Stat);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::readStatementParser(IRSyntaxNode *parentNode) { //scanf语句
    auto* kid = new IRSyntaxNode(Non_Read_Stat);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::printStatementParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Wirte_Stat);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
    ...
    if (peek() == '\"') {//说明即将要输出的是字符串
        ...
        addPrintNodeParser(strName, kid);//输出字符串中间代码
        flushStreamPos(ps);
        type = getNextSym(ans);
        if (type == COMMA) {
...
void SyntaxParser::addPrintNodeParser(string &strName, IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Print_Label);
    parentNode->addKid(kid);
    printStrIR(strName, parentNode);
}
...
void SyntaxParser::returnStatementParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Return_Stat);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::assignStatementParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Assign);
    parentNode->addKid(kid);
    string ans;
    string left;
    auto* ps = getCurPosition();
...
void SyntaxParser::conditionStatementParser(IRSyntaxNode *parentNode) { //条件语句
    auto* kid = new IRSyntaxNode(Non_Condition_Stat);
    parentNode->addKid(kid);
    string ans;
    ...
    addConditionalJumpParser(oriCodeLabel, condition, kid);//if condition goto label
    flushStreamPos(ps);
    type = getNextSym(ans);
    if (type != RPARENT) {
        callBackStreamPos(ps);
...
void SyntaxParser::conditionParser(string &condition, IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Condition);
    parentNode->addKid(kid);
    string left;
    string cmpOp;
    string right;
...
void SyntaxParser::recycleStatementParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Circle_Stat);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::mainFuncParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Main_Func);
    parentNode->addKid(kid);
    string ans;
    TokenType type = getNextSym(ans);
    if (type != VOIDTK) error();
    ...
    addFunctionNameLabelParser(ans, kid);//main函数中间代码标签
    type = getNextSym(ans);
    if (type != LPARENT) error();
    auto* ps = getCurPosition();
    type = getNextSym(ans);
...
ExprType SyntaxParser::expressionParser(string &ansSymbol, IRSyntaxNode *parentNode) {//表达式
    auto* kid = new IRSyntaxNode(Non_Expr);
    parentNode->addKid(kid);
...
ExprType SyntaxParser::itemParser(string &ansSymbol, IRSyntaxNode *parentNode) {//项
    auto* kid = new IRSyntaxNode(Non_Item);
    parentNode->addKid(kid);
...
ExprType SyntaxParser::factorParser(string &ansSymbol, IRSyntaxNode *parentNode) {//因子解析
    auto* kid = new IRSyntaxNode(Non_Factor);
    parentNode->addKid(kid);
    ExprType ansType = CharExp;
    string ans;
    ansSymbol.clear();
...
void SyntaxParser::returnFuncCallParser(string &ansSymbol, IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_ReturnFunc_Call);
    parentNode->addKid(kid);
...
void SyntaxParser::voidFuncCallParser(IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_VoidFunc_Call);
    parentNode->addKid(kid);
...
void SyntaxParser::funcCallValueListParser(string &functionName, IRSyntaxNode *parentNode) {
    auto* kid = new IRSyntaxNode(Non_Value_Table);
    parentNode->addKid(kid);
    string ans;
    auto* ps = getCurPosition();
    TokenType type = getNextSym(ans);
