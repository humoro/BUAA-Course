...
void SyntaxParser::unsignedIntParser() {
    string ans;
    TokenType type = getNextSym(ans);
...
void SyntaxParser::compoundStatementParser(string& functionName) {//复合语句的解析程序
    string ans;auto* ps = getCurPostion();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::conditionStatementParser() {
    ...
    else callBackStreamPos(ps);
}

void SyntaxParser::conditionParser() {
    ExprType exprType0 = expressionParser();
...
void SyntaxParser::stepStatementParser() {
    unsignedIntParser();
}
...
void SyntaxParser::stringParser() {
    string ans;
    TokenType  type = getNextSym(ans);
    if (type != STRCON) error();
}

void SyntaxParser::integerParser() {
    string ans;
    auto* ps = getCurPostion();
    TokenType type = getNextSym(ans);
    if (type == PLUS || type == MINU) {
...
ExprType SyntaxParser::expressionParser() {//表达式
...
ExprType SyntaxParser::itemParser() {//项
...
ExprType SyntaxParser::factorParser() {//因子解析
    ExprType ansType = CharExp;
    string ans;
    auto* ps = getCurPostion();
    TokenType type = getNextSym(ans);
...
void SyntaxParser::returnFuncCallParser() {
    funcCallParser();
}
...
void SyntaxParser::funcCallParser() {
    string ans;
    string functionName;
