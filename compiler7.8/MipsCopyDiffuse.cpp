        ...
        if (stmt->getIRType() == ir_assign) {
            ...
            if (left.find('[') == string::npos) { // 左侧的变量保证不能是数组数值
                for (auto j = i + 1;j < blk->size();j++) { // 把所有引用左侧变量的引用处修改为引用右侧变量
                    ...
                    if (blk->operator[](j)->getIRType() == ir_assign ||
                        blk->operator[](j)->getIRType() == ir_operation ||
                        blk->operator[](j)->getIRType() == ir_readChar ||
                        blk->operator[](j)->getIRType() == ir_readInt ||
                        blk->operator[](j)->getIRType() == ir_valueReturnFuncCall) { // 所有可能重定义变量的语句
                        if (blk->operator[](j)->getIRType() == ir_assign) {
                            getline(ns, copyleft, ' ');
                        ...
                        } else if (blk->operator[](j)->getIRType() == ir_readChar ||
                                   blk->operator[](j)->getIRType() == ir_readInt) {
                            getline(ns, copyleft, ' ');
                            getline(ns, copyright, ' ');
                        ...
                        if(blk->operator[](j)->getIRType() == ir_greJump || blk->operator[](j)->getIRType() == ir_geqJump ||
                           blk->operator[](j)->getIRType() == ir_lesJump || blk->operator[](j)->getIRType() == ir_leqJump ||
                           blk->operator[](j)->getIRType() == ir_eqlJump || blk->operator[](j)->getIRType() == ir_neqJmp) {
                            getline(ns, copyop, ' ');
                            getline(ns, copyopnum1, ' ');
                            getline(ns, copyopnum2, ' ');
                            getline(ns, copyans, ' ');
