#pragma once
#ifndef COMPILER7_2_PARSER_H
#define COMPILER7_2_PARSER_H
#include "Type.h"
#include "Signary.h"
#include "Error.h"
#include "IR.h"
...
private:
    ...
    string currentToken;
    TokenType currentTokenType = token_error;
...
private:
    ...
    void addFunctionNameLabelParser(string& functionName, string &typeString, IRSyntaxNode *parentNode);
    ...
    void addPrintNodeParser(string& strName, string &content, IRSyntaxNode *parentNode);
    ...
    bool entryExist(SymbolName& name) {
        return this->symbolTable->syntaxParse_entryExist(name);
    }

    bool isVoidFunc(SymbolName& name) {
        return this->symbolTable->syntaxParse_isVoidFunction(name);
    }

    bool isReturnFunc(SymbolName& name) {
        return this->symbolTable->syntaxParse_isReturnFunction(name);
    }

    bool isIntReturnFunc(SymbolName& name) {
        return this->symbolTable->syntaxParse_isIntReturnFunc(name);
    }

    bool isExistVar(SymbolName& name) {
        return this->symbolTable->syntaxParse_variableExist(name);
    }

    bool isConstVar(SymbolName& name) {
        return this->symbolTable->syntaxParse_isConstVariable(name);
    }

    bool isIntVar(SymbolName& name) {
        return this->symbolTable->syntaxParse_isIntVariable(name);
    }

    bool isIntArray(SymbolName& name) {
        return this->symbolTable->syntaxParse_isIntArray(name);
    }

    bool isCharArray(SymbolName& name) {
        return this->symbolTable->syntaxParse_isCharArray(name);
    }
    ...
    void addFunction(TokenType type, SymbolName& funcName) {
        if (this->symbolTable->syntaxParse_addFunction(type, funcName)) return;
        errorHandler(error_b);
    }

    void addFunctionParameter(TokenType varType, string& varName) {
        if (this->symbolTable->syntaxParse_addFunctionParameter(varType, varName)) return;
        errorHandler(error_b);
    }

    void addVariableEntry(TokenType varType, SymbolName& varName, bool isConst, long long value) {
        if (this->symbolTable->syntaxParse_addVariableEntry(varType, varName, isConst, value)) return;
        errorHandler(error_b);
    }

    void addArrayEntry(TokenType varType, SymbolName& varName, unsigned int size) {
        if (this->symbolTable->syntaxParse_addArrayEntry(varType, varName, size)) return;
        errorHandler(error_b);
    }

    void addFunctionReturn(ExprType type) {
        checkFunctionReturn(type);
        this->symbolTable->syntaxParse_getCurrentFunction()->addReturnStatement();
    }

    //------------------------------------------------check functions-------------------------------------------------//
    void checkFunctionReturn(ExprType type) {
        Function* curFunction = this->symbolTable->syntaxParse_getCurrentFunction();
        SymbolType sType = curFunction->getType();
        if (type == expr_none && (sType == symbol_intReturnFunction || sType == symbol_charReturnFunction)) {
            errorHandler(error_h);
            printf("(function name is: %s)\n\n",curFunction->getName().c_str());
        }
        if (type == expr_int && (sType == symbol_charReturnFunction)) {
            errorHandler(error_h);
            printf("(function name is: %s)\n\n",curFunction->getName().c_str());
        }
        if (type == expr_char && (sType == symbol_intReturnFunction)) {
            errorHandler(error_h);
            printf("(function name is: %s)\n\n",curFunction->getName().c_str());
        }
        if ((type == expr_int || type == expr_char) && sType == symbol_voidReturnFunction) {
            errorHandler(error_g);
            printf("(function name is: %s)\n\n",curFunction->getName().c_str());
        }
    }

    void checkReturnFunctionReturn() {
        auto* function = this->symbolTable->syntaxParse_getCurrentFunction();
        if (function->getType() != symbol_intReturnFunction && function->getType() != symbol_charReturnFunction) return;
        if ((!function->haveReturnStatement() &&
            (function->getType() == symbol_intReturnFunction || function->getType() == symbol_charReturnFunction))) {
            errorHandler(error_h);//返回函数没有返回语句
    ...
    void checkFunctionParameterList(string& functionName, vector<ExprType>& valueList) {
        ...
        if (!this->symbolTable->syntaxParse_getFunctionParaList(functionName, paras)) return;//获取函数参数列表失败
        if (paras.size() != valueList.size()) {//发现参数不匹配
            errorHandler(error_d);
            return;
        }
        for (unsigned long long i = 0;i < paras.size();i++) {
            if (!((valueList[i] == expr_int && paras[i]->getType() == symbol_intVariable) ||
                  (valueList[i] == expr_char && paras[i]->getType() == symbol_charVariable))) {
                errorHandler(error_e);//参数类型不匹配
                return;
            }
        }
    }
    ...
    void constDefIR(string &varName, string &typeString, int varValue, IRSyntaxNode *node) {//常量必须初始化并且变量全部是一维
        if (this->isglobal) {
            this->generator->globalConstDefIR(varName, typeString, varValue, node);
        } else {
            this->generator->partialConstDefIR(varName, typeString, varValue, node);
        }
    }

    void variableDefIR(string &varName, string &typeString, unsigned int size, IRSyntaxNode *node) {//变量定义的中间代码，最后的数值代表变量的规模
        if (this->isglobal) {
            this->generator->globalVariableDefIR(varName,typeString, size, node);
        } else {
            this->generator->partialVariableDefIR(varName, typeString, size, node);
        }
    }
    ...
    void functionDefIR(string &funcName, string &typeString, IRSyntaxNode *node) {
        this->generator->functionDef(funcName, typeString, node);
    }

    void functionDefParaIR(string &paraName, string &typeString, IRSyntaxNode *node) {
        this->generator->functionParaDef(paraName, typeString, node);
    }
    ...
    void readIR(SymbolName &name, IRSyntaxNode *node) {
        this->generator->read(this->symbolTable->syntaxParse_getSymbolType(name), name, node);
    }

    void printStrIR(string &str, string &content, IRSyntaxNode *node) {
        this->generator->printString(str, content, node);
    }
    ...
    string applyStringName(string& printStr) {
        return this->symbolTable->generateIR_applyStringName(printStr);
    }
...
public:
    explicit SyntaxParser(ifstream& inFile, SymbolTable* table) {
        ...
        this->root = new IRSyntaxNode(syntax_program);
    }
