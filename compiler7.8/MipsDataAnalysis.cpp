...
void IRTranslator::dataFlowInitial() {
    ...
    for (auto blk : this->blks) {
        ...
        for (auto stmt : blk->getInterCode()) {
            ...
            if (isUserDefinitionVariable(info1)) {
                blk->addUse(info1, stmt->getCodeNumber());
            }
            if (isUserDefinitionVariable(info2) && info4 != "[]") {
                blk->addUse(info2, stmt->getCodeNumber());
            }
            if (isUserDefinitionVariable(info3) && info4 != "[]") {
                blk->addUse(info3, stmt->getCodeNumber());
            }
        }
    }
}
    ...
    for (const auto& it : this->blkDefUseChainMap) {
        ...
        for (auto chain : it.second) {
            if (chain->getChain().empty()) {
                ...
                if (!this->entryTable->translateIR_isPartialEntry(left, function)) { // 全局变量的定义点不进行删除
                    continue;
                }
