...
static bool isCharConstantString(string& str) {
...
static bool isConstantString(string str) {//查看字符串是否代表常量
...
void IRTranslator::operationTranslator(IRStatement& statement) {
    ...
    if (op == "[]") { // 取数组元素
        if (this->table->isPartialVariable(opnum2, this->curFunctionName)) {
            ...
            if (isConstantString(opnum1)) { // index不可能单独的字符常量
                unsigned int indexnum;
                indexnum = stringToNumber(opnum1);
                indexnum = indexnum * 4;
    ...
    if (isConstantString(opnum1)) {
        if (opnum1.empty()) {//第一个操作数不存在只可能是加减
            opnum1 = "0";
        }
        if (opnum1.size() == 3 && (opnum1[0] == opnum1[2]) && (opnum1[0] == '\'')) {
            opnum1 = to_string(int(opnum1[1])); //操作数是字符常量
        }
    ...
    if (isConstantString(opnum2)) {
        if (opnum2.size() == 3 && (opnum2[0] == opnum2[2]) && (opnum2[0] == '\'')) {
            opnum2 = to_string(int(opnum2[1]));//操作数是字符常量
        }
...
void IRTranslator::funcRetAddrSwTranslator() {
    if (this->curFunctionName == "main") return;
