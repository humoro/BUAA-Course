...
void MipsGenerator::generateMips(bool needOptimal) {
    ...
    this->translator->dataAnalysis();
    if (needOptimal) {
        this->translator->deleteDeadCode();
        this->translator->activeVariableInitial();
        this->translator->activeVariableAnalysis();
        this->translator->buildConflictGraph();
    }
