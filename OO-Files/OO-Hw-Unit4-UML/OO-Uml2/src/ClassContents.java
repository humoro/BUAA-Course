...
class ClassContents {
    ...
    private HashMap<UmlInterface,
                    Integer> interfaceRealization = new HashMap<>();
    private List<String> realizationList = new ArrayList<>();
    private boolean isRefreshRealization = false;

    private HashMap<String,Integer> selfR002Name = new HashMap<>();
    private boolean isMultiInherent = false;
    ...
    void addAttribute(UmlAttribute attribute) {
        ...
        if (!this.selfR002Name.containsKey(name)) {
            this.selfR002Name.put(name,0);
        }
        int count = this.selfR002Name.get(name) + 1;
        this.selfR002Name.put(name,count);
    ...
    void addRealizationInterface(UmlInterface umlInterface,int count) {
        int time = count;
        if (this.interfaceRealization.containsKey(umlInterface)) {
            time = this.interfaceRealization.get(umlInterface) + count;
        }
        if (time >= 2) {
            this.isMultiInherent = true;
        }
        this.interfaceRealization.put(umlInterface,time);
    }
    ...
    boolean isMultiInherent() {
        return this.isMultiInherent;
    }

    HashMap<String,Integer> getSelfR002Name() {
        return this.selfR002Name;
    }
}
