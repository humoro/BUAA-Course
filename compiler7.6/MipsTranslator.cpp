...
void IRTranslator::constantDefinitionTranslator(IRStatement *statement) {
    ...
    if (statement->getIRType() == gConstDef) {
        generateMipsCode(symbol + ": .word " + value, globalData);
        this->entryTable->translateIR_setGlobalConstInfo(symbol, this->memAlloc, stringToNumber(value));//符号表中设置变量的地址
        this->memAlloc += 4;
    } else {
        this->entryTable->translateIR_setPartialConstInfo(symbol, this->curFunctionName, this->curStackAlloc, stringToNumber(value));
...
void IRTranslator::variableDefinitionTranslator(IRStatement *statement) {
    ...
    if (statement->getIRType() == gVarDef) {
        generateMipsCode(symbol + " : .space " + to_string(size * 4), globalData); // 全局的变量定义，声明地址空间
        if (size == 1) {
            this->entryTable->tranlasteIR_setGlobalVarAddr(symbol, this->memAlloc);
        } else {
            this->entryTable->translateIR_setGlobalArrayInfo(symbol, size, this->memAlloc);
        }
    } else {
        if (size == 1) {
            this->entryTable->translateIR_setPartialVarAddr(symbol, this->curFunctionName, this->curStackAlloc);
        } else {
...
void IRTranslator::programLabelSetTranslator(IRStatement* statement){//所有标签直接输出即可
    generateMipsCode(statement->getIR(), codeLabel);
}

void IRTranslator::funcDefLabelTranslator(IRStatement* statement){
    if (!switchText) {
        generateMipsCode(".text", segementSet);
        generateMipsCode("jal main",jFunction);
        generateMipsCode("j __end", jlabel);
        switchText = true;
    }
    stringstream ss(statement->getIR());
    string label;
    getline(ss,label,' ');
    getline(ss,label);
    generateMipsCode(label, codeLabel);
    //函数切换
    label.pop_back(); //弹掉冒号
...
void IRTranslator::funcRetAddrSwTranslator() {
    generateMipsCode("sw $ra,0($sp)", swRI); //存储返回地址
...
void IRTranslator::funcCallParaTranslator(IRStatement* statement){
    ...
    if (isConstVariable(paraVar)) {
        generateMipsCode("li $t8," + to_string(stringToNumber(paraVar)), li);
        reg = "t8";
    } else {
        reg = to_string(tmpRegisterPool(paraVar, true));
    }
    generateMipsCode("sw $" + reg + ",-" + to_string(this->pushParaStack) + "($sp)", swRI);
...
void IRTranslator::funcCallTranslator(IRStatement *statement, unsigned int beginloc) {
    ...
    sceneClear();
    sceneStore();
    generateMipsCode("sw $sp,0($gp)", swRI);
    generateMipsCode("addi $gp,$gp,4", addi);
    generateMipsCode("subi $sp,$sp," + to_string(this->sceneMemory), subi);
    generateMipsCode("jal " + functionName, jFunction);
    generateMipsCode("subi $gp,$gp,4", subi);
    generateMipsCode("lw $sp,0($gp)", lwRI);
    sceneLoad();//回复现场
    if (statement->getIRType() == retFuncCall) {
        unsigned int reg;
        reg = tmpRegisterPool(retVar, false); // 一个变量直接从寄存器中申请寄存器
        generateMipsCode("move $" + to_string(reg) + ",$v0", moveIN);
...
void IRTranslator::retStmtTranslator(IRStatement *statement) {
    if (statement->getIRType() == valueRet) {
        ...
        if (isConstVariable(retVar)) {
            ...
            generateMipsCode("li $v0," + retVar, li);
        } else {
            unsigned int reg = tmpRegisterPool(retVar, true);
            generateMipsCode("move $v0,$" + to_string(reg), moveIN);
        }
    }
    generateMipsCode("lw $ra,0($sp)", lwRI);
    generateMipsCode("jr $ra", jr);
}
...
void IRTranslator::readFunctionTranslator(IRStatement *statement) {
    ...
    generateMipsCode("li $v0," + syscallParameter, li);
    generateMipsCode("syscall", syscall);
    string variable;
    stringstream ss(statement->getIR());
    getline(ss, variable, ' ');
    getline(ss, variable, ' ');
    RegisterNumber reg = tmpRegisterPool(variable, false);
    generateMipsCode("move $" + to_string(reg) + ",$v0", moveIN);
...
void IRTranslator::printFunctionTranslator(IRStatement* statement) {
    ...
    if (statement->getIRType() == printStr) {
        generateMipsCode("la $a0," + variable, la);
        ...
        if (isConstVariable(variable)) {
            ...
            generateMipsCode("li $a0," + variable, li);
        } else {
            RegisterNumber reg = tmpRegisterPool(variable, true);
            generateMipsCode("move $a0,$" + to_string(reg), moveIN);
        }
    }
    string syscallParameter;
...
void IRTranslator::operationTranslator(IRStatement* statement) {
    ...
    string mips;
    if (op == "[]") { // 取数组元素
        if (this->entryTable->translateIR_isPartialVariable(opnum2, this->curFunctionName)) {
            ...
            if (isConstVariable(opnum1)) {//index不可能是单独的字符常量，此时下标是一个常量
                ...
                addr += indexnum * 4;
                generateMipsCode("lw $" + ansReg + ",-" + to_string(addr) + "($sp)", lwRI);
                ...
                vector<RegisterNumber> regs = applyTmpRegister(variables, needLoads);
                string regIndex = to_string(regs[1]);
                string ansReg = to_string(regs[0]);
                generateMipsCode("sll $t8,$" + regIndex + ",2", sll);
                generateMipsCode("addi $t8,$t8," + to_string(addr), addi);
                generateMipsCode("sub $t8,$sp,$t8", sub);
                generateMipsCode("lw $" + ansReg + ",0($t8)", lwRI);
                flushVariable(ansSym, stringToNumber(ansReg));
            }
        } else if (this->entryTable->translateIR_isGlobalVariable(opnum2)) {
            if (isConstVariable(opnum1)) {
                string ansReg = to_string(tmpRegisterPool(ansSym, false));
                unsigned int indexnum;
                indexnum = stringToNumber(opnum1);
                indexnum = indexnum * 4;
                generateMipsCode("lw $" + ansReg + "," + opnum2 + " + " + to_string(indexnum), lwLI);
                flushVariable(ansSym, stringToNumber(ansReg));
            } else {
                vector<string> variables = {ansSym, opnum1};
                vector<bool> needLoads = {false, true};
                ...
                string ansReg = to_string(regs[0]);
                generateMipsCode("sll $t8,$" + regIndex + ",2", sll);
                generateMipsCode("lw $" + ansReg + "," + opnum2 + "($t8)", lwRL);
                flushVariable(ansSym, stringToNumber(ansReg));
            }
        } else generateMipsCode("error", syscall);
        return;
    }
    if (opnum1.empty()) opnum1 = "0";
    if (isConstVariable(opnum1)) {
        opnum1 = to_string(stringToNumber(opnum1));
        generateMipsCode("li $t8," + opnum1, li);
        if (isConstVariable(opnum2)) {
            opnum2 = to_string(stringToNumber(opnum2));
            generateMipsCode("li $t9," + opnum2, li);
            RegisterNumber ansReg = tmpRegisterPool(ansSym, false);
            switch (op[0]) {
                case('+') :
                    generateMipsCode("add $" + to_string(ansReg) + ",$t8,$t9", add);
                    break;
                case('-') :
                    generateMipsCode("sub $" + to_string(ansReg) + ",$t8,$t9", sub);
                    break;
                case('*') :
                    generateMipsCode("mul $" + to_string(ansReg) + ",$t8,$t9", mul);
                    break;
                case('/') :
                    generateMipsCode("div $" + to_string(ansReg) + ",$t8,$t9", divid);
                    break;
                default:
                    generateMipsCode("error", syscall);
                    break;
            }
            ...
            switch (op[0]) {
                case('+') :
                    generateMipsCode("add $" + to_string(regs[1]) + ",$t8,$" + to_string(regs[0]), add);
                    break;
                case('-') :
                    generateMipsCode("sub $" + to_string(regs[1]) + ",$t8,$" + to_string(regs[0]), sub);
                    break;
                case('*') :
                    generateMipsCode("mul $" + to_string(regs[1]) + ",$t8,$" + to_string(regs[0]), mul);
                    break;
                case('/') :
                    generateMipsCode("div $" + to_string(regs[1]) + ",$t8,$" + to_string(regs[0]), divid);
                    break;
                default:
                    generateMipsCode("error", syscall);
                    break;
        ...
        if (isConstVariable(opnum2)) {
            ...
            switch (op[0]) {
                case('+') :
                    generateMipsCode("add $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + ",$t9", add);
                    break;
                case('-') :
                    generateMipsCode("sub $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + ",$t9", sub);
                    break;
                case('*') :
                    generateMipsCode("mul $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + ",$t9", mul);
                    break;
                case('/') :
                    generateMipsCode("div $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + ",$t9", divid);
                    break;
                default:
                    generateMipsCode("error", syscall);
                    break;
            ...
            switch (op[0]) {
                case('+') :
                    generateMipsCode("add $" + to_string(regs[2]) + ",$" + to_string(regs[0]) + ",$" + to_string(regs[1]),add);
                    break;
                case('-') :
                    generateMipsCode("sub $" + to_string(regs[2]) + ",$" + to_string(regs[0]) + ",$" + to_string(regs[1]),sub);
                    break;
                case('*') :
                    generateMipsCode("mul $" + to_string(regs[2]) + ",$" + to_string(regs[0]) + ",$" + to_string(regs[1]),mul);
                    break;
                case('/') :
                    generateMipsCode("div $" + to_string(regs[2]) + ",$" + to_string(regs[0]) + ",$" + to_string(regs[1]),divid);
                    break;
                default:
                    generateMipsCode("error", syscall);
                    break;
...
void IRTranslator::assignTranslator(IRStatement* statement){//同时赋值和改变寄存器
    ...
    if (isConstVariable(right)) { // 右侧是常量
        ...
        generateMipsCode("li $t9," + right, li); // t9作为中介寄存器，存放右侧数据的数值
        if (left.find('[') != std::string::npos) { // 左侧是数组
            ...
            if (isConstVariable(index)) { //索引是常量
                ...
                if (this->entryTable->translateIR_isPartialVariable(array, this->curFunctionName)) {
                    unsigned int addr = this->entryTable->translateIR_getPartialArrayAddr(array, this->curFunctionName);
                    addr += indexNumber * 4;
                    generateMipsCode("sw $t9,-" + to_string(addr) + "($sp)", swRI);
                } else if (this->entryTable->translateIR_isGlobalVariable(array)) {
                    indexNumber *= 4;
                    generateMipsCode("sw $t9," + array + "+" + to_string(indexNumber), swLI);
                } else generateMipsCode("error", syscall);
            } else { //索引是变量
                RegisterNumber indexValueReg = tmpRegisterPool(index, true);
                generateMipsCode("sll $t8,$" + to_string(indexValueReg) + ",2", sll); //index左移两位
                if (this->entryTable->translateIR_isPartialVariable(array, this->curFunctionName)) {
                    unsigned int addr = this->entryTable->translateIR_getPartialArrayAddr(array, this->curFunctionName);
                    generateMipsCode("addi $t8,$t8," + to_string(addr), addi); //index寄存器中地址先加上数组的基地址
                    generateMipsCode("sub $t8,$sp,$t8", sub); //由于是在栈中，为了下面使用寄存器直接寻址做准备
                    generateMipsCode("sw $t9,0($t8)", swRI);
                } else if (this->entryTable->translateIR_isGlobalVariable(array)) {
                    generateMipsCode("sw $t9," + array + ",($t8)", swRL);
                } else generateMipsCode("error", syscall);
            }
        } else {
            RegisterNumber leftReg = tmpRegisterPool(left, false); //为左侧变量申请一个寄存器
            generateMipsCode("move $" + to_string(leftReg) + ",$t9", moveIN);
        ...
        if (left.find('[') != std::string::npos) { // 左侧是数组
            ...
            #endif
            if (isConstVariable(index)) { //索引是常量
                RegisterNumber rightValueReg = tmpRegisterPool(right, true);
                unsigned int indexNumber = stringToNumber(index);
                if (this->entryTable->translateIR_isPartialVariable(array, this->curFunctionName)) {
                    unsigned int addr = this->entryTable->translateIR_getPartialArrayAddr(array, this->curFunctionName);
                    addr += indexNumber * 4;
                    generateMipsCode("sw $" + to_string(rightValueReg) + ",-" + to_string(addr) + "($sp)", swRI);
                } else if (this->entryTable->translateIR_isGlobalVariable(array)) {
                    indexNumber *= 4;
                    generateMipsCode("sw $" + to_string(rightValueReg) + "," + array + "+" + to_string(indexNumber),swLI);
                } else generateMipsCode("error", syscall);
            } else {
                ...
                if (this->entryTable->translateIR_isPartialVariable(array, this->curFunctionName)) {
                    ...
                    generateMipsCode("addi $t8,$t8," + to_string(addr), addi); //index寄存器中地址先加上数组的基地址
                    generateMipsCode("sub $t8,$sp,$t8", sub); //由于是在栈中，为了下面使用寄存器直接寻址做准备
                    generateMipsCode("sw $" + to_string(rightValueReg) + ",0($t8)", swRI);
                } else if (this->entryTable->translateIR_isGlobalVariable(array)) {
                    generateMipsCode("sw $" + to_string(rightValueReg) + "," + array + ",($t8)", swRL);
                } else generateMipsCode("error", syscall);
            }
        } else { //左侧是变量
            ...
            vector<bool> needLoads = {false, true};
            vector<RegisterNumber> regs = applyTmpRegister(variables, needLoads);
            generateMipsCode("move $" + to_string(regs[0]) + ",$" + to_string(regs[1]), moveIN);
...
void IRTranslator::conditionJumpTranslator(IRStatement *statement) {
    ...
    if (isConstVariable(left) || isConstVariable(right)) {
        if (isConstVariable(left) && isConstVariable(right)) { // 两个比较数都是常数
            if (right == "0") {
                ...
                generateMipsCode("li $t8," + left, li); // t8是左侧比较数
                ...
                generateMipsCode(cmpMips + " $t8," + label, bz);
            } else {
                if (left == "0") {
                    //注意此时有左侧数是0所以比较语句要颠倒
                    right = to_string(stringToNumber(right));
                    generateMipsCode("li $t8," + right, li); // t8是右侧比较数
                    if (statement->getIRType() == geqJump) { // 大于等于跳转
                        cmpMips = "blez";
                    } else if (statement->getIRType() == greJump) { // 大于跳转
                        cmpMips = "bltz";
            ...
            if (right == "0") {
                ...
                generateMipsCode(cmpMips + " $" + left + "," + label, bz);
            } else if (left == "0"){
                ...
                generateMipsCode(cmpMips + " $" + right + "," + label, bz);
            } else { // 没有常数0
                if (isConstVariable(right)) { // 右侧数是常数
                    ...
                    generateMipsCode(cmpMips + " $" + left + "," + right + "," + label, br);
                } else { // 左侧数是常数
                    right = to_string(tmpRegisterPool(right, true)); // 右侧侧是变量获取其寄存器
                    left = to_string(stringToNumber(left));
                    if (statement->getIRType() == geqJump) { // 大于等于跳转
                    ...
                    generateMipsCode(cmpMips + " $" + right + "," + left + "," + label, br);
                }
            }
        }
        return;
    ...
    vector<bool> needLoads = {true, true};
    vector<RegisterNumber> regs = applyTmpRegister(cmps, needLoads);
    string regl,regr;
    ...
    generateMipsCode(cmpMips + " $" + regl + ",$" + regr + "," + label, br);
}
...
void IRTranslator::jumpTranslator(IRStatement* statement){//无条件跳转，使用j指令直接跳转
    ...
    getline(ss,label,' ');
    getline(ss,label,' ');
    generateMipsCode("j " + label, jlabel);
}
