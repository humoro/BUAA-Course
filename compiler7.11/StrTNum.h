...
public:
    ...
    bool isConstant(string &str, string& functionName) {//查看字符串是否代表常量
        if (isCharConstantString(str) || isNumberConstantString(str)) return true;
        if (this->entryTable->translateIR_isPartialEntry(str, functionName)) {
            return this->entryTable->translateIR_isPartialConstant(str, functionName);
        }
        return (this->entryTable->translateIR_isGlobalConstant(str));
    }
