...
bool IRTranslator::isUserDefinitionVariable(string &variable, string &functionName) { // 必须满足：用户定义，变量（不是常量或者常数，不是数组）
    return !variable.empty() && variable.find('[') == string::npos &&
           !isConstVariable(variable,functionName) && variable.find('@') == string::npos; // 不是中间变量并且不是数组
}

bool IRTranslator::variableAfterUser(string& name, unsigned int beginLoc) {
