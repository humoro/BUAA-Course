#include <sstream>
#include "Mips.h"
static unsigned int scale = 30; // 中间代码小于15并且是叶子函数才进行函数内联
...
bool IRTranslator::isInlineCrash(string &callee, string &caller, string &variable) {
    return (!this->entryTable->translateIR_isPartialEntry(variable, callee) &&
             this->entryTable->translateIR_isPartialEntry(variable, caller));
}

bool IRTranslator::inlineFunction(string &callee) {
    for (auto blk : this->functionBlks[callee]) {
        for (auto stmt : this->blockNumberBlkMap[blk]->getIntermediateCode()) {
            stringstream ss(stmt->getIR());
            string variable, op, opnum1, opnum2, ans;
            switch(stmt->getIRType()) {
                //函数相关：
                case ir_valueReturn:
                {
                    getline(ss, variable, ' ');
                    getline(ss, variable, ' '); // 得到返回值返回变量
                    if (!this->entryTable->translateIR_isPartialEntry(variable,callee) &&
                        this->entryTable->translateIR_isGlobalEntry(variable)) return false;
                }
                    break;
                case ir_greJump:
                case ir_geqJump:
                ...
                case ir_neqJmp:
                {
                    ...
                    getline(ss, opnum2, ' ');
                    getline(ss, ans, ' ');
                    if (!this->entryTable->translateIR_isPartialEntry(opnum1,callee) &&
                        this->entryTable->translateIR_isGlobalEntry(opnum1)) return false;
                    if (!this->entryTable->translateIR_isPartialEntry(opnum2,callee) &&
                        this->entryTable->translateIR_isGlobalEntry(opnum2)) return false;
                }
                    break;
                case ir_readInt:
                case ir_readChar:
                ...
                case ir_operation:
                {
                    ...
                    if (!this->entryTable->translateIR_isPartialEntry(opnum2,callee) &&
                        this->entryTable->translateIR_isGlobalEntry(opnum2)) return false;
                    if (!this->entryTable->translateIR_isPartialEntry(ans, callee) &&
                        this->entryTable->translateIR_isGlobalEntry(ans)) return false;
                }
                    break;
                case ir_assign:
                {
                    ...
                    if (opnum1.find('[') != string::npos) { // 左侧是数组
                        ...
                        getline(ns, array, '['); //获取数组标识符
                        getline(ns, index, ']'); //获取下标标识符
                        if (!this->entryTable->translateIR_isPartialEntry(array,callee) &&
                            this->entryTable->translateIR_isGlobalEntry(array)) return false;
                    } else { // 左侧不是数组
                        if (!this->entryTable->translateIR_isPartialEntry(opnum1,callee) &&
                            this->entryTable->translateIR_isGlobalEntry(opnum1)) return false;
                    }
...
void IRTranslator::functionInlineOptimal() {
    ...
    for (const auto& it : this->functionBlks) {
        ...
        for (auto number : it.second) {
            for (auto stmt : this->blockNumberBlkMap[number]->getIntermediateCode()) {
                ...
                if (stmt->getIRType() == ir_funcCallBegin) { // 有跳转语句也不能进行内联，因为可能出现跨越基本块的变量
                    string callFunction;
                    stringstream ss(stmt->getIR());
        ...
        this->inlineFunctionMap[function] = inlineFunction(function);
    }
    ...
    for (auto& it : this->functionCallMap) {
        ...
        for (const auto& call : it.second) {
            callee = call;
            if (!this->inlineFunctionMap[call] ||
        ...
        for (unsigned int i = 0;i < this->functionStatementsMap[it.first].size();i++) {
            ...
            if (stmt->getIRType() == ir_funcCallBegin) {
                ...
                for (i++; this->functionStatementsMap[it.first][i]->getIRType() != ir_valueReturnFuncCall &&
                          this->functionStatementsMap[it.first][i]->getIRType() != ir_voidReturnFuncCall; i++, paraIndex++) {
                    //参数传递修改为赋值语句
