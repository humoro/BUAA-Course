...
void IRTranslator::dataFlowInitial() {
    ...
    for (auto it : this->blockNumberBlkMap) {
        it.second->clearDataFlowGenKill(); // 下面的函数开始计算每个基本块的gen和kill
    }
    ...
    for (const auto& it : this->functionBlks) {
        for (auto blkNumber : it.second) {
            ...
            for (auto* stmt : blk->getIntermediateCode()) { // 先初始化各个语句的gen数据,并存储得到的定义点信息和变量
                ...
                if (isUserDefinitionVariable(genVariable, function)) { // 是用户定义变量，那么产生定义信息
    ...
    for (auto it : this->functionBlks) {
        ...
        for (auto blk : it.second) {
            ...
            auto gen = this->blockNumberBlkMap[blk]->getDataFlowGen();
            for (auto item : gen) {
                cout << item << " ";
            }
            cout << endl;
    ...
    for (auto blk : this->blks) {
        ...
        for (auto stmt : blk->getIntermediateCode()) {
            ...
            if (isUserDefinitionVariable(variable, function)) {
                blk->addUse(variable, stmt->getCodeNumber());
            }
            if (isUserDefinitionVariable(opnum1, function) && op != "[]") {
                blk->addUse(opnum1, stmt->getCodeNumber());
            }
            if (isUserDefinitionVariable(opnum2, function) && op != "[]") {
...
void IRTranslator::dataFlowAnalysis() {
    for (const auto& it : this->functionBlks) {
        ...
        while (cycleSwitch) { // 当有基本块的数据流信息有变化的时候不断计算
            ...
            for (auto blknumber : it.second) {
                ...
                outSet = dataFlowUnion(this->blockNumberBlkMap[blknumber]->getDataFlowGen(), dataFlowDiff(inSet,this->blockNumberBlkMap[blknumber]->getDataFlowKill()));
    ...
    for (auto it : this->blkBlockNumberMap) {
        ...
        cout << "in" <<  endl;
        for (auto node : it.first->getDataFlowIn()) {
            cout << node << " ";
        }
        cout << endl;
        cout << "out" << endl;
        for (auto node : it.first->getDataFlowOut()) {
            cout << node << " ";
        }
        cout << endl;
    }
...
void IRTranslator::deleteDeadCode() {
    ...
    for (const auto& function : this->functions) {
        for (auto blk : this->functionBlks[function]) {
            for (auto stmt : this->blockNumberBlkMap[blk]->getIntermediateCode()) {
                this->statements.push_back(stmt);
            }
        }
    ...
    for (const auto& it : this->functionBlks) {
        for (auto number : it.second) {
            ...
            for (auto stmt : blk->getIntermediateCode()) {
                ...
                string function = it.first;
                if (isUserDefinitionVariable(right, function)) {
                    if (defSet.find(right) == defSet.end()) { // 使用先于所有定义
                        useSet.insert(right);
                    }
                }
                if (isUserDefinitionVariable(left, function)) { // 产生的定义先于所有使用
                ...
                if (isUserDefinitionVariable(ans, function)) { // 产生的定义先于所有使用
                    if (useSet.find(ans) == useSet.end()) {
                        defSet.insert(ans);
                    }
                }
                if (isUserDefinitionVariable(useVariable, function)) {
                    if (defSet.find(useVariable) == defSet.end()) { // 使用先于所有定义
                        useSet.insert(useVariable);
                    }
                }
                if (isUserDefinitionVariable(defVariable, function)) { // 产生的定义先于所有使用
