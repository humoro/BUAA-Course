...
void IRTranslator::funcCallParaTranslator(IRStatement *statement, unsigned int beginLoc) {
    string paraVar;
    stringstream ss(statement->getIR());
    getline(ss,paraVar,' ');
    getline(ss,paraVar,' ');
    ...
    if (isConstVariable(paraVar)) {
        ...
        reg = to_string(applyRegister(paraVar, true, beginLoc));
    }
...
void IRTranslator::funcCallTranslator(IRStatement *statement, unsigned int beginloc) {
    ...
    if (statement->getIRType() == ir_valueReturnFuncCall) {
        RegisterNumber reg;
        reg = applyRegister(retVar, false, beginloc); // 一个变量直接从寄存器中申请寄存器
        generateMipsCode("move $" + to_string(reg) + ",$v0", mips_moveReg);
        flushVariable(retVar, reg);
    }
}

void IRTranslator::retStmtTranslator(IRStatement *statement, unsigned int beginLoc) {
    if (statement->getIRType() == ir_valueReturn) {
        ...
        if (isConstVariable(retVar)) {
            ...
            RegisterNumber reg = applyRegister(retVar, true, beginLoc);
...
void IRTranslator::readFunctionTranslator(IRStatement *statement, unsigned int beginLoc) {
    ...
    getline(ss, variable, ' ');
    getline(ss, variable, ' ');
    RegisterNumber reg = applyRegister(variable, false, beginLoc);
    generateMipsCode("move $" + to_string(reg) + ",$v0", mips_moveReg);
    flushVariable(variable, reg); // 读到的变量更新到内存中
}

void IRTranslator::printFunctionTranslator(IRStatement *statement, unsigned int beginLoc) {
    string variable;
    stringstream ss(statement->getIR());
    getline(ss, variable, ' ');
    getline(ss, variable, ' ');
    if (statement->getIRType() == ir_printStr) {
        ...
        if (isConstVariable(variable)) {
            ...
            RegisterNumber reg = applyRegister(variable, true, beginLoc);
...
//表达式计算中没有改变变量的数值
void IRTranslator::operationTranslator(IRStatement *statement, unsigned int beginLoc) {
    ...
    if (op == "[]") { // 取数组元素
        if (this->entryTable->translateIR_isPartialEntry(opnum2, this->curFunctionName)) {
            ...
            if (isConstVariable(opnum1)) {
                ...
                indexnum = this->stoi(opnum1);
                indexnum = indexnum * 4;
                generateMipsCode("lw $" + ansReg + "," + opnum2 + " + " + to_string(indexnum) + "($0)", mips_lwLabelImm);
                flushVariable(ansSym, std::stoi(ansReg));
            } else {
                vector<string> variables = {ansSym, opnum1};
                vector<bool> needLoads = {false, true};
    ...
    if (isConstVariable(opnum1)) {
        ...
        if (isConstVariable(opnum2)) {
            ...
            RegisterNumber ansReg = applyRegister(ansSym, false, beginLoc);
            switch (op[0]) {
                case('+') :
                    generateMipsCode("add $" + to_string(ansReg) + ",$t8," + opnum2, mips_addReg);
                    break;
                case('-') :
                    generateMipsCode("sub $" + to_string(ansReg) + ",$t8," + opnum2, mips_subReg);
                    break;
                case('*') :
                    generateMipsCode("mul $" + to_string(ansReg) + ",$t8," + opnum2, mips_mulImm);
                    break;
                case('/') :
                    generateMipsCode("div $" + to_string(ansReg) + ",$t8," + opnum2, mips_divImm);
                    break;
                default:
        ...
        if (isConstVariable(opnum2)) {
            ...
            switch (op[0]) {
                case('+') :
                    generateMipsCode("add $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + "," + opnum2, mips_addReg);
                    break;
                case('-') :
                    generateMipsCode("sub $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + "," + opnum2, mips_subReg);
                    break;
                case('*') :
                    generateMipsCode("mul $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + "," + opnum2, mips_mulImm);
                    break;
                case('/') :
                    generateMipsCode("div $" + to_string(regs[1]) + ",$" + to_string(regs[0]) + "," + opnum2, mips_divImm);
                    break;
                default:
                    generateMipsCode("error", mips_syscall);
                    break;
...
void IRTranslator::assignTranslator(IRStatement *statement, unsigned int beginLoc) {//同时赋值和改变寄存器
    stringstream ss(statement->getIR());
    string left,right;
    getline(ss,left,' ');
    getline(ss,right,' ');
    ...
    if (isConstVariable(right)) { // 右侧是常量
        ...
        if (left.find('[') != std::string::npos) { // 左侧是数组
            ...
            if (isConstVariable(index)) { //索引是常量
                ...
                if (this->entryTable->translateIR_isPartialEntry(array, this->curFunctionName)) {
                    ...
                    indexNumber *= 4;
                    generateMipsCode("sw $t9," + array + " + " + to_string(indexNumber) + "($0)", mips_swLabelImm);
                } else generateMipsCode("error", mips_syscall);
            } else { //索引是变量
                RegisterNumber indexValueReg = applyRegister(index, true, beginLoc);
            ...
            RegisterNumber leftReg = applyRegister(left, false, beginLoc); //为左侧变量申请一个寄存器
        ...
        if (left.find('[') != std::string::npos) { // 左侧是数组
            ...
            #endif
            if (isConstVariable(index)) { //索引是常量
                RegisterNumber rightValueReg = applyRegister(right, true, beginLoc);
                unsigned int indexNumber = this->stoi(index);
                if (this->entryTable->translateIR_isPartialEntry(array, this->curFunctionName)) {
                    unsigned int addr = this->entryTable->translateIR_getPartialArrayAddr(array, this->curFunctionName);
                    addr += indexNumber * 4;
                    ...
                    indexNumber *= 4;
                    generateMipsCode("sw $" + to_string(rightValueReg) + "," + array + " + " + to_string(indexNumber) + "($0)", mips_swLabelImm);
                } else generateMipsCode("error", mips_syscall);
            } else {
...
void IRTranslator::conditionJumpTranslator(IRStatement *statement, unsigned int beginLoc) {
    ...
    if (isConstVariable(left) || isConstVariable(right)) {
        if (isConstVariable(left) && isConstVariable(right)) { // 两个比较数都是常数
            ...
            if (right == "0") {
                left = to_string(applyRegister(left, true, beginLoc)); // 左侧是变量获取其寄存器
                if (statement->getIRType() == ir_geqJump) { // 大于等于跳转
                    cmpMips = "bgez";
                } else if (statement->getIRType() == ir_greJump) { // 大于跳转
                    cmpMips = "bgtz";
                ...
                right = to_string(applyRegister(right, true, beginLoc)); // 右侧侧是变量获取其寄存器
                if (statement->getIRType() == ir_geqJump) { // 大于等于跳转
                    cmpMips = "blez";
                } else if (statement->getIRType() == ir_greJump) { // 大于跳转
                    cmpMips = "bltz";
    ...
    vector<bool> needLoads = {true, true};
    vector<RegisterNumber> regs = applyRegister(cmps, needLoads, beginLoc);
    string regl,regr;
    ...
    for (unsigned int i = 0;i < blk->size();i++) {
        ...
        switch (statement->getIRType()) {
            ...
            case ir_funcCallParaPush:
                funcCallParaTranslator(statement, i);
                break;
            ...
            case ir_readInt:
            case ir_readChar:
                readFunctionTranslator(statement, i);
                break;
            case ir_printNewLine:
            case ir_printStr:
            case ir_printChar:
            case ir_printInt:
                printFunctionTranslator(statement, i);
                break;
            case ir_operation:
                operationTranslator(statement, i);
                break;
            case ir_assign:
                assignTranslator(statement, i);
                break;
            case ir_greJump:
            case ir_geqJump:
            case ir_lesJump:
            ...
            case ir_eqlJump:
            case ir_neqJmp:
                conditionJumpTranslator(statement, i); // 条件跳转
                break;
            case ir_jump:
