...
bool IRTranslator::flushVariable(string& varName, RegisterNumber valueReg) {
    if (isGlbRegister(valueReg)) { // 全局变量只需要标识不需要再从内存中初始化数值
        ...
        if (this->entryTable->translateIR_isPartialEntry(varName, this->curFunctionName)) { //该变量在栈中分配了内存
            storeToStack(varName, valueReg);
            return true;
        }
        if (this->entryTable->translateIR_isGlobalEntry(varName)) {
...
void IRTranslator::storeToStack(string& varName, RegisterNumber valueReg) {
    unsigned int addr = this->entryTable->translateIR_getPartialVariableAddr(varName, this->curFunctionName);
    generateMipsCode("sw $" + to_string(valueReg) + ",-" + to_string(addr) + "($sp)", mips_swRegImm);
}

void IRTranslator::storeToMemory(string& varName, RegisterNumber valueReg) {//为用户的全局变量设计
    generateMipsCode("sw $" + to_string(valueReg) + "," + varName + "($0)", mips_swRegLabel);
}

void IRTranslator::storeToMemory(RegisterNumber reg, unsigned int addr) {
    generateMipsCode("sw $" + to_string(reg) + "," + to_string(addr) + "($0)", mips_swRegImm);
}
...
void IRTranslator::loadFromStack(string &varName, RegisterNumber valueReg) {
    if (isTmpRegister(valueReg) && !this->entryTable->translateIR_isPartialVariableRefreshed(varName,
                                                                                             this->curFunctionName)) return;
    if (isGlbRegister(valueReg) && this->loadedGlbVariable.find(varName) != this->loadedGlbVariable.end()) return;
    unsigned int addr = this->entryTable->translateIR_getPartialVariableAddr(varName, this->curFunctionName);
    generateMipsCode("lw $" + to_string(valueReg) + ",-" + to_string(addr) + "($sp)", mips_lwRegImm);
...
void IRTranslator::loadFromMemory(string &varName, RegisterNumber valueReg) { //这是已经全局声明内存的变量提取
    if (isTmpRegister(valueReg) && !this->entryTable->translateIR_isGlobalVariableRefreshed(varName)) return;
    if (isGlbRegister(valueReg) && this->loadedGlbVariable.find(varName) != this->loadedGlbVariable.end()) return;
    generateMipsCode("lw $" + to_string(valueReg) + "," + varName + "($0)", mips_lwRegLabel);
...
void IRTranslator::loadFromMemory(RegisterNumber reg, unsigned int addr) {
    generateMipsCode("lw $" + to_string(reg) + "," + to_string(addr) + "($0)", mips_lwRegImm);
}
